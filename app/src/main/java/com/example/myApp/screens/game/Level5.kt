...
@Composable
fun Level5 (puzzleId: String? = "tt0499549", navController: NavController = rememberNavController(), difficulty: Int = 0, imageID: Int = 0) {
    ...
    Scaffold(
        ...
        MainContentLevel5(puzzle, difficulty, imageID, navController)
    }
}
...
fun MainContentLevel5(puzzle: PuzzleList, difficulty: Int, imageID: Int, navController: NavController) {
    ...
    val b: Bitmap =
        BitmapFactory.decodeResource(LocalContext.current.resources, R.drawable.level5)
    val splittedBitmap = splitImageLevel5(b,colum*colum)
    ...
    val state = rememberReorderableLazyGridState(onMove = {from, to ->
        data.value = data.value.toMutableList().apply {
            swapLevel5(to.index, from.index)
        }
    })

    if (isGameOverLevel5(data.value) && statikLevel5.count == 0) {// If Sorted
        data.value = data.value.toMutableList().apply { //Randomise
            shuffle()
        }
    }

    statikLevel5() //For Closing Game

    if (isGameOverLevel5(data.value) && statikLevel5.count > 0) {
        popUpWindowLevel5(navController)
    }
...
@Composable
fun popUpWindowLevel5(navController: NavController){
...
fun isGameOverLevel5(data: List<Int>): Boolean {
...
fun <T> MutableList<T>.swapLevel5(index1: Int, index2: Int){
    val tmp = this[index1]
    this[index1] = this[index2]
    this[index2] = tmp
}

fun splitImageLevel5(image: Bitmap, chunkNumbers: Int): ArrayList<Bitmap>{
