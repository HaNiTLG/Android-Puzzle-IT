...
@Composable
fun Level2 (puzzleId: String? = "tt0499549", navController: NavController = rememberNavController(), difficulty: Int = 0, imageID: Int = 0) {
    ...
    Scaffold(
        ...
        MainContentLevel2(puzzle, difficulty, imageID, navController)
    }
}
@SuppressLint("UnusedMaterialScaffoldPaddingParameter", "UnrememberedMutableState")
@Composable
fun MainContentLevel2(puzzle: PuzzleList, difficulty: Int, imageID: Int, navController: NavController) {
    ...
    val b: Bitmap =
        BitmapFactory.decodeResource(LocalContext.current.resources, R.drawable.level2)
    val splittedBitmap = splitImageLevel2(b,colum*colum)
    ...
    val state = rememberReorderableLazyGridState(onMove = {from, to ->
        data.value = data.value.toMutableList().apply {
            swapLevel2(to.index, from.index)
        }
    })

    if (isGameOverLevel2(data.value) && statikLevel2.count == 0) {// If Sorted
            data.value = data.value.toMutableList().apply { //Randomise
                shuffle()
            }
    }

    statikLevel2() //For Closing Game

    if (isGameOverLevel2(data.value) && statikLevel2.count > 0) {
        popUpWindowLevel2(navController)
    }
...
@Composable
fun popUpWindowLevel2(navController: NavController){
...
fun isGameOverLevel2(data: List<Int>): Boolean {
...
fun <T> MutableList<T>.swapLevel2(index1: Int, index2: Int){
    val tmp = this[index1]
    this[index1] = this[index2]
    this[index2] = tmp
}

fun splitImageLevel2(image: Bitmap, chunkNumbers: Int): ArrayList<Bitmap>{
