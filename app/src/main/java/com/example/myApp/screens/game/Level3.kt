...
@Composable
fun Level3 (puzzleId: String? = "tt0499549", navController: NavController = rememberNavController(), difficulty: Int = 0, imageID: Int = 0) {
    ...
    Scaffold(
        ...
        MainContentLevel3(puzzle, difficulty, imageID, navController)
    }
}
@SuppressLint("UnusedMaterialScaffoldPaddingParameter", "UnrememberedMutableState")
@Composable
fun MainContentLevel3(puzzle: PuzzleList, difficulty: Int, imageID: Int, navController: NavController) {
    ...
    val b: Bitmap =
        BitmapFactory.decodeResource(LocalContext.current.resources, R.drawable.level3)
    val splittedBitmap = splitImageLevel3(b,colum*colum)
    ...
    val state = rememberReorderableLazyGridState(onMove = {from, to ->
        data.value = data.value.toMutableList().apply {
            swapLevel3(to.index, from.index)
        }
    })

    if (isGameOverLevel3(data.value) && statikLevel3.count == 0) {// If Sorted
        data.value = data.value.toMutableList().apply { //Randomise
            shuffle()
        }
    }

    statikLevel3() //For Closing Game

    if (isGameOverLevel3(data.value) && statikLevel3.count > 0) {
        popUpWindowLevel3(navController)
    }
...
@Composable
fun popUpWindowLevel3(navController: NavController){
...
fun isGameOverLevel3(data: List<Int>): Boolean {
...
fun <T> MutableList<T>.swapLevel3(index1: Int, index2: Int){
    val tmp = this[index1]
    this[index1] = this[index2]
    this[index2] = tmp
}

fun splitImageLevel3(image: Bitmap, chunkNumbers: Int): ArrayList<Bitmap>{
